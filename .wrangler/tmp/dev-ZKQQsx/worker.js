...
__name(getGameConfig, "getGameConfig");
function getAllGames() {
  return Object.values(GAME_CONFIGS);
}
__name(getAllGames, "getAllGames");
function getTeamModeGames() {
  return Object.values(GAME_CONFIGS).filter((g) => g.supportsTeamMode);
}
__name(getTeamModeGames, "getTeamModeGames");
function getEffectiveArchetype(gameType, useCustomPoints) {
  if (useCustomPoints) return "points";
  return GAME_CONFIGS[gameType].defaultArchetype;
}
__name(getEffectiveArchetype, "getEffectiveArchetype");

// shared/gameArchetypes.ts
var SCORE_ARCHETYPES = {
  rounds: {
    id: "rounds",
    name: "Rounds",
  ...
  kills: {
    ...
    scoreLabel: "Kills",
    tiesPossible: true,
    // Ties possible in timed matches
    loserHasScore: true,
    higherIsBetter: true,
  ...
  winonly: {
    ...
    scoreLabel: "Result",
    tiesPossible: true,
    // Ties can be agreed upon in group stage
    loserHasScore: false,
    higherIsBetter: true,
    requiresMaxScore: false,
...
__name(getArchetypeConfig, "getArchetypeConfig");

// shared/validation.ts
var WIN_POINTS = 3;
var DRAW_POINTS = 1;
var LOSS_POINTS = 0;
var VALID_POINTS = [LOSS_POINTS, DRAW_POINTS, WIN_POINTS];
function isFiniteNumber(v) {
  return typeof v === "number" && Number.isFinite(v);
}
__name(isFiniteNumber, "isFiniteNumber");
function validateMatchResult(results, participants, archetype) {
  ...
  const isDecided = a === WIN_POINTS && b === LOSS_POINTS || a === LOSS_POINTS && b === WIN_POINTS;
  if (!isDraw && !isDecided) {
...
__name(validateMatchResult, "validateMatchResult");
function validateTeamMatchResult(team1Score, team2Score, archetype) {
...
__name(validateTeamMatchResult, "validateTeamMatchResult");

// shared/access.ts
var ALPHABET = "0123456789ABCDEFGHJKMNPQRSTVWXYZ";
var JOIN_CODE_LENGTH = 6;
var ADMIN_KEY_LENGTH = 26;
...
__name(generateAdminKey, "generateAdminKey");
function normalizeCode(input) {
  if (typeof input !== "string") return "";
  let out = "";
  for (const raw2 of input.toUpperCase()) {
    const ch = raw2 === "I" || raw2 === "L" ? "1" : raw2 === "O" ? "0" : raw2;
...
async function verifyAdminKey(key, hash) {
  if (!key || !hash) return false;
  return timingSafeEqualHex(await hashAdminKey(key), hash);
}
__name(verifyAdminKey, "verifyAdminKey");
function pathTournamentId(path) {
  if (typeof path !== "string") return null;
  const m = path.match(/^\/tournament\/([^/?#]+)/);
  return m ? m[1] : null;
}
__name(pathTournamentId, "pathTournamentId");

// server/brackets.ts
function build3PlayerFinalsBracket(players) {
  return [{
    id: v4_default(),
    round: 1,
    bracketType: "finals",
...
__name(build3PlayerFinalsBracket, "build3PlayerFinalsBracket");
function buildDirectFinalsBracket(players) {
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
  return [
    {
      id: thirdPlaceId,
      round: 1,
...
__name(buildDirectFinalsBracket, "buildDirectFinalsBracket");
function build4PlayerSemifinalsBracket(players) {
  const semi1Id = v4_default();
  const semi2Id = v4_default();
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
  return [
    {
...
__name(build4PlayerSemifinalsBracket, "build4PlayerSemifinalsBracket");
function build6PlayerBracket(players) {
  const quarter1Id = v4_default();
  const quarter2Id = v4_default();
  const semi1Id = v4_default();
  const semi2Id = v4_default();
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
...
__name(build6PlayerBracket, "build6PlayerBracket");
function build8PlayerBracket(players) {
  const quarter1Id = v4_default();
  const quarter2Id = v4_default();
  const quarter3Id = v4_default();
  const quarter4Id = v4_default();
  const semi1Id = v4_default();
  const semi2Id = v4_default();
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
...
__name(build8PlayerBracket, "build8PlayerBracket");
function buildPlayoffBracket(players, totalTournamentPlayers) {
...
__name(buildPlayoffBracket, "buildPlayoffBracket");
function reorderForCrossGroupMatchups(players, numGroups, groupOf) {
  if (numGroups <= 1) return players;
  const playersByGroup = /* @__PURE__ */ new Map();
  ...
  const interleave = /* @__PURE__ */ __name(() => {
    const reordered = [];
...
__name(reorderForCrossGroupMatchups, "reorderForCrossGroupMatchups");
function build3TeamFinalsBracket(teams) {
  return [{
    id: v4_default(),
    round: 1,
    bracketType: "finals",
...
__name(build3TeamFinalsBracket, "build3TeamFinalsBracket");
function buildDirectTeamFinalsBracket(teams) {
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
  return [
    {
...
__name(buildDirectTeamFinalsBracket, "buildDirectTeamFinalsBracket");
function build4TeamBracket(teams) {
  const semi1Id = v4_default();
  const semi2Id = v4_default();
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
...
__name(build4TeamBracket, "build4TeamBracket");
function build6TeamBracket(teams) {
  const qf1Id = v4_default();
  const qf2Id = v4_default();
  const semi1Id = v4_default();
  const semi2Id = v4_default();
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
...
__name(build6TeamBracket, "build6TeamBracket");
function build8TeamBracket(teams) {
  const qf1Id = v4_default();
  const qf2Id = v4_default();
  const qf3Id = v4_default();
  const qf4Id = v4_default();
  const semi1Id = v4_default();
  const semi2Id = v4_default();
  const thirdPlaceId = v4_default();
  const finalId = v4_default();
...
__name(build8TeamBracket, "build8TeamBracket");
function buildTeamPlayoffBracket(teams, totalTournamentTeams, numGroups) {
...
__name(buildTeamPlayoffBracket, "buildTeamPlayoffBracket");
...
var TournamentManager = class _TournamentManager {
  static {
    __name(this, "TournamentManager");
  }
  id;
  name;
  gameType;
  ...
  mapPool = [];
  // Optional map pool for tournaments
  players = [];
  pods = [];
  matches = [];
  bracketMatches = [];
  ...
  joinCode = "";
  adminKeyHash = "";
  groupStageRoundLimit;
  // Custom round limit for group stage (CS 1.6)
  playoffsRoundLimit;
  // Custom round limit for playoffs (CS 1.6)
  useCustomPoints;
  // Override default archetype with custom points
  // Team tournament support
  _schemaVersion = 1;
  // 1 = solo, 2 = team-capable
  isTeamBased = false;
  teams = [];
  teamPods = [];
  teamMatches = [];
  teamBracketMatches = [];
  constructor(id, name, gameType = "cs16", mapPool = [], groupStageRoundLimit, playoffsRoundLimit, useCustomPoints, teamMode) {
    this.id = id;
    this.name = name;
  ...
  static fromJSON(data) {
    ...
    const t = new _TournamentManager(
      data.id,
      typeof data.name === "string" ? data.name : "Untitled",
      data.gameType,
      Array.isArray(data.mapPool) ? data.mapPool : [],
      typeof data.groupStageRoundLimit === "number" ? data.groupStageRoundLimit : void 0,
      typeof data.playoffsRoundLimit === "number" ? data.playoffsRoundLimit : void 0,
    ...
    const asArray = /* @__PURE__ */ __name((v) => Array.isArray(v) ? v : [], "asArray");
  ...
  fillMissingMaps() {
    ...
    let changed = false;
    const mapFor = /* @__PURE__ */ __name((seed) => {
      let h = 2166136261;
    ...
    for (const match2 of this.matches) {
      if (!match2.mapName && match2.completed) {
        match2.mapName = mapFor(match2.id);
        changed = true;
      }
    }
    for (const match2 of this.bracketMatches) {
      for (const game of match2.games ?? []) {
        if (!game.mapName) {
          game.mapName = mapFor(`${match2.id}:${game.gameNumber}`);
          changed = true;
        }
      }
    }
  ...
  addTeam(name, playerIds, logo) {
    ...
    const config = this.getGameConfig();
    if (config.minTeamSize !== void 0 && playerIds.length < config.minTeamSize) {
      throw new Error(`Team must have at least ${config.minTeamSize} players`);
    }
    if (config.maxTeamSize !== void 0 && playerIds.length > config.maxTeamSize) {
  ...
  updateTeam(teamId, updates) {
    ...
    if (updates.logo !== void 0) {
      validateImage(updates.logo, "logo");
      team.logo = updates.logo;
    }
    if (updates.playerIds) {
      ...
      const config = this.getGameConfig();
      if (config.minTeamSize !== void 0 && updates.playerIds.length < config.minTeamSize) {
        throw new Error(`Team must have at least ${config.minTeamSize} players`);
      }
      if (config.maxTeamSize !== void 0 && updates.playerIds.length > config.maxTeamSize) {
        throw new Error(`Team can have at most ${config.maxTeamSize} players`);
      }
  ...
  generatePods() {
    ...
    const numGroups = _TournamentManager.GROUP_COUNTS[numPlayers] ?? Math.ceil(numPlayers / 4);
    ...
    groups.forEach((groupPlayers, groupIndex) => {
      const podId = v4_default();
      this.pods.push({
        id: podId,
        round: 1,
        // All belong to the same logical group
  ...
  createDirectFinals2Players(players) {
    const finalId = v4_default();
  ...
  submitMatchResult(matchId, results, mapName, gameResults) {
    ...
    const match2 = this.matches.find((m) => m.id === matchId);
    if (!match2) throw new Error("Match not found");
    validateMatchResult(results, [match2.player1Id, match2.player2Id], this.getArchetype());
    match2.result = results;
    match2.completed = true;
    if (mapName) {
      match2.mapName = mapName;
    }
    if (gameResults) {
      match2.gameResults = gameResults;
    }
  ...
  recalculatePlayerStats() {
    ...
    for (const match2 of this.matches) {
      if (!match2.completed || !match2.result) continue;
      for (const [playerId, result] of Object.entries(match2.result)) {
        const player = this.players.find((p) => p.id === playerId);
        if (!player) continue;
        player.matchesPlayed++;
        player.points += result.points;
        if (result.points >= 3) player.wins++;
        else if (result.points === 1) player.draws++;
        else player.losses++;
        if (result.score !== void 0) {
          player.totalGameScore += result.score;
          const opponentId = match2.player1Id === playerId ? match2.player2Id : match2.player1Id;
          const opponentResult = match2.result[opponentId];
          if (opponentResult?.score !== void 0) {
  ...
  submitBracketGameResult(matchId, gameResult) {
    const match2 = this.bracketMatches.find((m) => m.id === matchId);
    if (!match2) throw new Error("Bracket match not found");
    ...
    if (!match2.games) match2.games = [];
    const existing = match2.games.findIndex((g) => g.gameNumber === gameResult.gameNumber);
    if (existing === -1) match2.games.push(gameResult);
    else match2.games[existing] = gameResult;
    match2.games.sort((a, b) => a.gameNumber - b.gameNumber);
    match2.player1Wins = match2.games.filter((g) => g.winnerId && g.winnerId === match2.player1Id).length;
    match2.player2Wins = match2.games.filter((g) => g.winnerId && g.winnerId === match2.player2Id).length;
    const winsNeeded = Math.ceil(maxGames / 2);
    if (match2.player1Wins >= winsNeeded) {
      this.submitBracketWinner(matchId, match2.player1Id);
    } else if (match2.player2Wins >= winsNeeded) {
      this.submitBracketWinner(matchId, match2.player2Id);
    }
  }
  getRankings() {
  ...
  getHeadToHeadResult(playerAId, playerBId) {
    const directMatch = this.matches.find(
      (m) => m.completed && (m.player1Id === playerAId && m.player2Id === playerBId || m.player1Id === playerBId && m.player2Id === playerAId)
    );
  ...
  submitBracketWinner(matchId, winnerId) {
    const match2 = this.bracketMatches.find((m) => m.id === matchId);
    if (!match2) throw new Error("Match not found");
    if (match2.player1Id !== winnerId && match2.player2Id !== winnerId) {
      throw new Error("Winner must be one of the players in the match");
    }
    if (match2.winnerId === winnerId) return;
    match2.winnerId = winnerId;
    const loserId = match2.player1Id === winnerId ? match2.player2Id : match2.player1Id;
    if (match2.nextMatchId) {
      const nextMatch = this.bracketMatches.find((m) => m.id === match2.nextMatchId);
      if (nextMatch) {
        if (match2.nextMatchSlot === 1) {
    ...
    if (match2.bracketType === "semifinals") {
  ...
  resetGroupData(podId) {
    if (this.isTeamBased) {
      ...
      this.teamMatches.forEach((match2) => {
        if (match2.podId === podId) {
          match2.team1Score = void 0;
          match2.team2Score = void 0;
          match2.winnerId = void 0;
          match2.games = void 0;
          match2.completed = false;
        }
      });
      return;
    }
    ...
    this.matches.forEach((match2) => {
      if (match2.podId === podId) {
        match2.result = void 0;
        match2.completed = false;
        match2.mapName = void 0;
      }
    });
  }
  // Update player name
  ...
  generateTeamPods() {
    ...
    const numGroups = numTeams <= 7 ? 1 : numTeams === 8 ? 2 : numTeams <= 12 ? 3 : 4;
      ...
      for (let round = 0; round < totalRounds; round++) {
        for (let i = 0; i < matchesPerRound; i++) {
          ...
          if (t1 && t2) {
            ...
            this.teamMatches.push(match2);
            if (!pod.matchId) pod.matchId = match2.id;
          }
        }
        if (n % 2 === 0) {
  ...
  createDirectTeamFinals2Teams(teams) {
    const finalId = v4_default();
  ...
  submitTeamMatchResult(matchId, team1Score, team2Score, games) {
    ...
    const match2 = this.teamMatches.find((m) => m.id === matchId);
    if (!match2) throw new Error("Team match not found");
    validateTeamMatchResult(team1Score, team2Score, this.getArchetype());
    match2.team1Score = team1Score;
    match2.team2Score = team2Score;
    match2.games = games;
    match2.completed = true;
    if (team1Score > team2Score) {
      match2.winnerId = match2.team1Id;
    } else if (team2Score > team1Score) {
      match2.winnerId = match2.team2Id;
    }
  ...
  recalculateTeamStats() {
    ...
    this.teamMatches.filter((m) => m.completed).forEach((match2) => {
      const team1 = this.teams.find((t) => t.id === match2.team1Id);
      const team2 = this.teams.find((t) => t.id === match2.team2Id);
      if (!team1 || !team2) return;
      team1.matchesPlayed++;
      team2.matchesPlayed++;
      team1.roundsWon += match2.team1Score || 0;
      team1.roundsLost += match2.team2Score || 0;
      team2.roundsWon += match2.team2Score || 0;
      team2.roundsLost += match2.team1Score || 0;
      if (match2.winnerId === team1.id) {
        team1.wins++;
        team1.points += 3;
        team2.losses++;
      } else if (match2.winnerId === team2.id) {
        team2.wins++;
        team2.points += 3;
        team1.losses++;
      } else {
  ...
  submitTeamBracketGameResult(matchId, gameResult) {
    const match2 = this.teamBracketMatches.find((m) => m.id === matchId);
    if (!match2) throw new Error("Team bracket match not found");
    const maxGames = this.getMapsPerMatch();
    if (!Number.isInteger(gameResult.gameNumber) || gameResult.gameNumber < 1 || gameResult.gameNumber > maxGames) {
      throw new Error(`Game number must be between 1 and ${maxGames}`);
    }
    ...
    match2.games.sort((a, b) => a.gameNumber - b.gameNumber);
    match2.team1Wins = match2.games.filter((g) => g.winnerTeamId && g.winnerTeamId === match2.team1Id).length;
    match2.team2Wins = match2.games.filter((g) => g.winnerTeamId && g.winnerTeamId === match2.team2Id).length;
    const winsNeeded = Math.ceil(maxGames / 2);
    if (match2.team1Wins >= winsNeeded) {
      this.submitTeamBracketWinner(matchId, match2.team1Id);
    } else if (match2.team2Wins >= winsNeeded) {
      this.submitTeamBracketWinner(matchId, match2.team2Id);
    }
  }
  // Submit the winner of a team bracket match
  submitTeamBracketWinner(matchId, winnerTeamId) {
    const match2 = this.teamBracketMatches.find((m) => m.id === matchId);
    if (!match2) throw new Error("Team bracket match not found");
    if (match2.team1Id !== winnerTeamId && match2.team2Id !== winnerTeamId) {
      throw new Error("Winner must be one of the teams in the match");
    }
    if (match2.winnerId === winnerTeamId) return;
    match2.winnerId = winnerTeamId;
    const loserId = match2.team1Id === winnerTeamId ? match2.team2Id : match2.team1Id;
    if (match2.nextMatchId) {
      const nextMatch = this.teamBracketMatches.find((m) => m.id === match2.nextMatchId);
      if (nextMatch) {
        if (match2.nextMatchSlot === 1) {
  ...
  generateTeamBrackets() {
    ...
    const qualifiedTeamIds = /* @__PURE__ */ new Set();
  ...
  getTeamRankings() {
    ...
    this.teamMatches.filter((m) => m.completed).forEach((match2) => {
      const stats1 = teamStats.get(match2.team1Id);
      const stats2 = teamStats.get(match2.team2Id);
      const score1 = match2.team1Score || 0;
      const score2 = match2.team2Score || 0;
      stats1.roundsWon += score1;
      stats1.roundsDiff += score1 - score2;
      stats2.roundsWon += score2;
      stats2.roundsDiff += score2 - score1;
      if (match2.winnerId === match2.team1Id) {
        stats1.points += 3;
        stats1.wins += 1;
      } else if (match2.winnerId === match2.team2Id) {
        stats2.points += 3;
        stats2.wins += 1;
      } else {
        stats1.points += 1;
  ...
  getPlayerStatistics() {
    ...
    this.teamMatches.filter((m) => m.completed && m.games).forEach((match2) => {
      match2.games.forEach((game) => {
    ...
    this.teamBracketMatches.filter((m) => m.games).forEach((match2) => {
      match2.games.forEach((game) => {
        game.playerStats?.forEach((ps) => {
          const stats = playerStats.get(ps.playerId);
          if (stats) {
...
// server/store/withTournament.ts
var TournamentNotFoundError = class extends Error {
  static {
    __name(this, "TournamentNotFoundError");
  }
  constructor(id) {
...
var MAX_ATTEMPTS = 12;
var BASE_BACKOFF_MS = 10;
var MAX_BACKOFF_MS = 200;
async function withTournament(store, id, mutate) {
  let lastConflict;
...
__name(withTournament, "withTournament");

// server/app.ts
var MUTATING = ["POST", "PUT", "DELETE", "PATCH"];
var RATE_WINDOW_MS = 15 * 60 * 1e3;
function makeRateLimiter(maxAttempts) {
...
__name(makeRateLimiter, "makeRateLimiter");
function reportTokenHealth(adminToken) {
  if (!adminToken) {
    console.warn(
...
__name(reportTokenHealth, "reportTokenHealth");
function createApiApp({ store, adminToken }) {
  const app = new Hono2();
  const tokenLimiter = makeRateLimiter(10);
  const joinLimiter = makeRateLimiter(30);
  let tokenHash = null;
  const adminTokenHash = /* @__PURE__ */ __name(() => tokenHash ??= hashSecret(adminToken), "adminTokenHash");
  const clientKey = /* @__PURE__ */ __name((c) => c.req.header("cf-connecting-ip") || (c.req.header("x-forwarded-for") || "").split(",")[0].trim() || "unknown", "clientKey");
  const body = /* @__PURE__ */ __name(async (c) => {
    try {
  ...
  async function isInstanceOwner(c) {
  ...
  __name(isInstanceOwner, "isInstanceOwner");
  async function canWriteTournament(c, tournamentId) {
    if (await isInstanceOwner(c)) return true;
  ...
  __name(canWriteTournament, "canWriteTournament");
  const fail = /* @__PURE__ */ __name((c, err) => {
  ...
  __name(freshJoinCode, "freshJoinCode");
  ...
  app.delete("/api/tournament/:tournamentId", async (c) => {
    ...
    try {
      ...
      return c.json({ error: "Failed to delete tournament", details: err.message }, 500);
    }
  });
  app.post("/api/tournaments/import", async (c) => {
    try {
      ...
      return c.json({ error: "Failed to import tournament", details: err.message }, 500);
    }
  });
  ...
  const mutate = /* @__PURE__ */ __name((fn) => async (c) => {
    try {
  ...
  const mutateReturning = /* @__PURE__ */ __name((fn) => async (c) => {
    try {
      const b = await body(c);
  ...
  app.put(
    "/api/tournament/:tournamentId/player/:playerId",
    mutate((c, t, b) => t.updatePlayerName(c.req.param("playerId"), b.name))
  );
  app.put(
    "/api/tournament/:tournamentId/player/:playerId/photo",
  ...
  app.post(
    "/api/tournament/:tournamentId/match/:id",
    mutate((c, t, b) => t.submitMatchResult(c.req.param("id"), b.results, b.mapName))
  );
  app.put(
    "/api/tournament/:tournamentId/group/:podId/name",
    mutate((c, t, b) => t.updateGroupName(c.req.param("podId"), b.name))
  );
  app.post(
    "/api/tournament/:tournamentId/group/:podId/reset",
    mutate((c, t) => t.resetGroupData(c.req.param("podId")))
  );
  app.post(
    "/api/tournament/:tournamentId/team-match/:id",
    mutate((c, t, b) => t.submitTeamMatchResult(c.req.param("id"), b.team1Score, b.team2Score, b.games))
  );
  app.post(
    "/api/tournament/:tournamentId/brackets",
    mutate((_c, t) => {
      t.isTeamBased ? t.generateTeamBrackets() : t.generateBrackets();
    })
  );
  app.post(
    "/api/tournament/:tournamentId/team-brackets",
    mutate((_c, t) => t.generateTeamBrackets())
  );
  app.post(
    "/api/tournament/:tournamentId/bracket-match/:id",
    mutate((c, t, b) => t.submitBracketWinner(c.req.param("id"), b.winnerId))
  );
  app.post(
    "/api/tournament/:tournamentId/bracket-match/:id/game",
  ...
  app.post(
    "/api/tournament/:tournamentId/team-bracket-match/:id",
    mutate((c, t, b) => t.submitTeamBracketWinner(c.req.param("id"), b.winnerId))
  );
  app.post(
    "/api/tournament/:tournamentId/team-bracket-match/:id/game",
    mutate((c, t, b) => {
      const game = {
        gameNumber: b.gameNumber,
        mapName: b.mapName,
  ...
  app.post("/api/tournament/:tournamentId/teams", async (c) => {
    ...
    try {
      const { result } = await withTournament(
        store,
        c.req.param("tournamentId"),
        (t) => t.addTeam(name, playerIds, logo)
      );
      return c.json(result);
    } catch (err) {
      return fail(c, err);
  ...
  app.put(
    "/api/tournament/:tournamentId/team/:teamId",
  ...
  app.put(
    "/api/tournament/:tournamentId/team/:teamId/logo",
    mutate((c, t, b) => {
      t.updateTeam(c.req.param("teamId"), { logo: b.logo });
    })
  );
  app.delete(
    "/api/tournament/:tournamentId/team/:teamId",
  ...
  app.get("/api/tournament/:tournamentId/player-stats", async (c) => {
    ...
    return c.json(
      Array.from(loaded.tournament.getPlayerStatistics().entries()).map(([playerId, data]) => ({ playerId, ...data }))
    );
  });
...
__name(assertRecoveryCodesExperimentalEnabled, "assertRecoveryCodesExperimentalEnabled");
...
__name(checkTopLevelSchemaOption, "checkTopLevelSchemaOption");
...
var SUMMARY_COLUMNS = "id,name,join_code,game_type,state,is_team_based,player_count,created_at,started_at";
var SupabaseTournamentStore = class {
  static {
    __name(this, "SupabaseTournamentStore");
  }
  db;
  ...
  async init() {
    ...
    if (error) {
      throw new Error(
        `Cannot read the tournaments table (${error.message}). Apply supabase/schema.sql and check SUPABASE_SERVICE_ROLE_KEY.`
      );
    }
  }
  async close() {
  ...
  hydrate(row) {
    ...
    try {
      ...
      console.error(`[supabase] unreadable tournament ${row.id}:`, err.message);
      return null;
    }
  }
  async load(id) {
    const { data, error } = await this.db.from("tournaments").select("*").eq("id", id).maybeSingle();
    if (error) throw new Error(error.message);
    return this.hydrate(data);
  }
  async loadByJoinCode(code) {
    const { data, error } = await this.db.from("tournaments").select("*").eq("join_code", code).maybeSingle();
    if (error) throw new Error(error.message);
    return this.hydrate(data);
  }
  async listSummaries() {
    const { data, error } = await this.db.from("tournaments").select(SUMMARY_COLUMNS).order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    return (data ?? []).map((r) => ({
      id: r.id,
      name: r.name,
  ...
  async insert(t) {
    ...
    if (error) throw new Error(error.message);
    const { error: codeError } = await this.db.from("retired_join_codes").insert({ code: t.joinCode });
    if (codeError && codeError.code !== "23505") throw new Error(codeError.message);
  }
  async update(t, expectedVersion) {
    ...
    if (error) throw new Error(error.message);
    if (data === null || data === void 0) throw new VersionConflictError(t.id);
    return Number(data);
  }
  async remove(id) {
  ...
  async isJoinCodeTaken(code) {
    const { data, error } = await this.db.from("retired_join_codes").select("code").eq("code", code).maybeSingle();
    if (error) throw new Error(error.message);
    return data !== null;
  }
  async isNameTaken(name) {
    const { data, error } = await this.db.from("tournaments").select("id").ilike("name", name.trim()).limit(1);
    if (error) throw new Error(error.message);
...
function getApp(env) {
  ...
  if (!env.SUPABASE_URL || !env.SUPABASE_SERVICE_ROLE_KEY) {
    throw new Error(
      "SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set. See the Cloudflare section of the README."
    );
  }
...
// .wrangler/tmp/bundle-SgJeye/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SgJeye/middleware-loader.entry.ts
